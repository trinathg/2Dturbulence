...
void bcs(vector<vertex>& node,vector<fval>& fvar)
{	
	int ind; 
...
/**********************************************************************************/ 
//Periodic boundary conditions for Taylor Green flow 

void per_bcs(vector<vertex>& node,vector<fval>& fvar)
{
	int ind; 
 	 ...
 	 for(int i=0;i<=nx;i=i+1)
	 {
		...
		fvar[ind].u[0] = fvar[i].u[0];
		fvar[ind].u[1] = fvar[i].u[1];
		fvar[ind].u[2] = fvar[i].u[2]; 			
	 } 
	 ...
	 for(int j=0;j<=ny;j=j+1)
	 {
		...
		fvar[ind].u[0] = fvar[j*str_x].u[0];
		fvar[ind].u[1] = fvar[j*str_x].u[1];
		fvar[ind].u[2] = fvar[j*str_x].u[2]; 
	 }	
} 
/************************************************************************************/ 
