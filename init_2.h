...
const int nx_per=128, ny_per=128, nz_per=20; 
const int nx=nx_per+1, ny=ny_per+1, nz=nz_per+1;  //No. of spacings in X, Y and Z directions for the computational domainconst int str_x = nx+1; 
const int str_x = nx+1; 
  
const int file_freq=100; // Frequency of writing down files for data visualization
const int r_file_freq = 3*file_freq; //Frequency for wriitng the restart file

const double l_y = 2.0*M_PI; //Non-dimensional extents of computational domain for Taylor-Green problem
const double l_x = 2.0*M_PI;

const double delta_p = 2.0; //This is the difference between the outlet and inlet pressure. The flow is from left to right

const double Re = 60.0; //Reynolds number based on the size of the cavity 
...
/************Parameters for the simulation of 2D turbulence************/

const double b_ratio = 131.;  //Box ratio
const double l_int = l_x/b_ratio; //Integral length scale
const double kp = 1./l_int; //Wave number where the maximum energy of the spectrum occurs. E(kp) = 1
const double Q = exp(4)/pow(kp,8); //The normaliing coefficient for the spectrum of the form E(k) = Q(k^8)exp(-4.*(k/kp)^2)
const int kx_min = -(nx_per/2)+1,kx_max=(nx_per/2);
const int ky_min = -(ny_per/2)+1,ky_max=(ny_per/2);

const double eddy_turn_time = 1./b_ratio; //Eddy turn over time
const double dt = 1.0e-3; //CFL_d*(dx)*(dx)*Re; //1/Re acts as a diffusion coefficient here 
const double tot_time = 10.*eddy_turn_time; //Total non-dimensional simulation time
const int ite = tot_time/dt; //No. of iterations till the end of the simulation time
...
typedef struct mg
{
	double phi_s[tot_p], F[tot_p], res[tot_p], coeff[tot_p], cor_rhs[tot_p], rhs[tot_p], p1x[ny+1], p2x[ny+1], p1y[nx+1], p2y[nx+1]; 
	double point_correc; 
}mg_grid; 

extern std::vector<double> hx, hy, hx2, hy2, hxx, hyy; //The grid sizes at various levels of the multigrid cycle
extern std::vector<double> X1,X2,X3,X4,X5,X6,X7,X8,X9; 
extern std::vector<double> Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8,Y9;
...
extern std::vector<double> dc_ny1_11, dc_ny1_12, dc_ny1_13, dc_ny1_21, dc_ny1_22, dc_ny1_23, dc_ny1_24, dc_ny1_32, dc_ny1_33, dc_ny1_34, dc_ny1_35, dc_ny1_36;
extern std::vector<double> dc_ny_11, dc_ny_12, dc_ny_13, dc_ny_21, dc_ny_22, dc_ny_23, dc_ny_24, dc_ny_32, dc_ny_33, dc_ny_34, dc_ny_35, dc_ny_36;
...
extern std::vector<double> dc_pb7_11, dc_pb7_12, dc_pb7_13, dc_pb7_21, dc_pb7_22, dc_pb7_23, dc_pb7_24, dc_pb7_32, dc_pb7_33, dc_pb7_34, dc_pb7_35, dc_pb7_36; 
